...
export interface VerifiedSignedExtensionRequest {
...
export function verifySignedExtensionRequest(
...
): VerifiedSignedExtensionRequest {
  if (signature === null || signature.length === 0) {
    throw new SignedExtensionRequestError("SIGNATURE_MISSING", "Stripe-Signature is required");
  }
  ...
  try {
    ...
    throw new SignedExtensionRequestError("SIGNATURE_INVALID", "The signed request is invalid");
  }
  ...
  try {
    ...
    throw new SignedExtensionRequestError("ENVELOPE_INVALID", "Request body must be valid JSON");
  }

  const parsed = signedEnvelopeSchema.safeParse(json);
  if (!parsed.success) {
    throw new SignedExtensionRequestError("ENVELOPE_INVALID", "Signed request envelope is invalid");
  }
